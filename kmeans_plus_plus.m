...
for run=1:runs
    ...
    centroids(1,:)=X(unidrnd(m),:);
    nc=1;
    while nc<K
        distmat=zeros(m,nc);
        for cdv=1:nc
            d=bsxfun(@minus,X,centroids(cdv,:));
            d=d.*d;
            distmat(:,cdv)=sum(d,2);
        end
        finaldistmat=min(distmat,[],2);
        probvec=bsxfun(@rdivide,finaldistmat,sum(finaldistmat));
        newcentroidindex=find(mnrnd(1,probvec));
        nc=nc+1;
        centroids(nc,:)=X(newcentroidindex,:);
    end
